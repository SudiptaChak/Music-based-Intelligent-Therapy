...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
  ...
  public void collectInitialPrefetchPositions(int paramInt, RecyclerView.LayoutManager.LayoutPrefetchRegistry paramLayoutPrefetchRegistry) {
    ...
    boolean bool = false;
    int j = i;
    for (int i = bool; i < this.mInitialPrefetchItemCount && j >= 0 && j < paramInt; i++) {
    ...
    for (paramRecycler = null; paramInt1 != paramInt2; paramRecycler = recycler) {
      ...
      if (k >= 0) {
        ...
        if (k < paramInt3)
          ...
          } else if (this.mOrientationHelper.getDecoratedStart(view4) >= i || this.mOrientationHelper.getDecoratedEnd(view4) < j) {
            state = paramState;
            recycler = paramRecycler;
            if (paramState == null) {
              view3 = view4;
    ...
    if (paramState.isPreLayout()) {
      ...
      if (i1 != -1) {
        ...
        if (this.mPendingScrollPositionOffset != Integer.MIN_VALUE) {
          ...
          if (view != null) {
            if (this.mShouldReverseLayout) {
              i = this.mOrientationHelper.getEndAfterPadding() - this.mOrientationHelper.getDecoratedEnd(view);
              j = this.mPendingScrollPositionOffset;
            } else {
              j = this.mOrientationHelper.getDecoratedStart(view) - this.mOrientationHelper.getStartAfterPadding();
    ...
    public void assignFromViewAndKeepVisibleRect(View param1View, int param1Int) {
      ...
      if (this.mLayoutFromEnd) {
        ...
        if (param1Int > 0) {
          ...
          i = this.mCoordinate;
          int j = this.mOrientationHelper.getStartAfterPadding();
          i = i - k - j + Math.min(this.mOrientationHelper.getDecoratedStart(param1View) - j, 0);
          if (i < 0)
        ...
        this.mCoordinate = j;
        if (param1Int > 0) {
          int n = this.mOrientationHelper.getDecoratedMeasurement(param1View);
          int k = this.mOrientationHelper.getEndAfterPadding();
          int m = this.mOrientationHelper.getDecoratedEnd(param1View);
          i = this.mOrientationHelper.getEndAfterPadding() - Math.min(0, k - i - m) - j + n;
          if (i < 0)
            this.mCoordinate -= Math.min(param1Int, -i); 
        } 
      } 
...
/* Location:              C:\Users\admin\Desktop\Test\dex-tools-v2.4\classes-dex2jar.jar!\androidx\recyclerview\widget\LinearLayoutManager.class
