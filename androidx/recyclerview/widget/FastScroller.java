...
class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
  ...
  void updateScrollPosition(int paramInt1, int paramInt2) {
    boolean bool;
    int k = this.mRecyclerView.computeVerticalScrollRange();
    int j = this.mRecyclerViewHeight;
    if (k - j > 0 && j >= this.mScrollbarMinimumRange) {
      bool = true;
    } else {
      bool = false;
    } 
    ...
    if (m - i > 0 && i >= this.mScrollbarMinimumRange) {
      bool = true;
    } else {
      bool = false;
    } 
    ...
    if (this.mNeedVerticalScrollbar) {
      float f1 = paramInt2;
      float f2 = j;
      this.mVerticalThumbCenterY = (int)(f2 * (f1 + f2 / 2.0F) / k);
      this.mVerticalThumbHeight = Math.min(j, j * j / k);
    } 
    if (this.mNeedHorizontalScrollbar) {
      float f1 = paramInt1;
      float f2 = i;
      this.mHorizontalThumbCenterX = (int)(f2 * (f1 + f2 / 2.0F) / m);
      this.mHorizontalThumbWidth = Math.min(i, i * i / m);
    } 
...
/* Location:              C:\Users\admin\Desktop\Test\dex-tools-v2.4\classes-dex2jar.jar!\androidx\recyclerview\widget\FastScroller.class
