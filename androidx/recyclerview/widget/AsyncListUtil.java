...
public class AsyncListUtil<T> {
  ...
  private final ThreadUtil.BackgroundCallback<T> mBackgroundCallback = new ThreadUtil.BackgroundCallback<T>() {
      ...
      private void flushTileCache(int param1Int) {
        ...
        while (this.mLoadedTiles.size() >= i) {
          int k = this.mLoadedTiles.keyAt(0);
          SparseBooleanArray sparseBooleanArray = this.mLoadedTiles;
          int m = sparseBooleanArray.keyAt(sparseBooleanArray.size() - 1);
          int j = this.mFirstRequiredTileStart - k;
          int n = m - this.mLastRequiredTileStart;
          if (j > 0 && (j >= n || param1Int == 2)) {
            removeTile(k);
            continue;
          } 
          if (n > 0 && (j < n || param1Int == 1))
            removeTile(m); 
        } 
      }
  ...
  void updateRange() {
    ...
    if (arrayOfInt[0] <= arrayOfInt[1] && arrayOfInt[0] >= 0) {
      ...
      backgroundCallback.updateRange(i, j, arrayOfInt[0], arrayOfInt[1], this.mScrollHint);
    } 
  }
...
/* Location:              C:\Users\admin\Desktop\Test\dex-tools-v2.4\classes-dex2jar.jar!\androidx\recyclerview\widget\AsyncListUtil.class
