...
public class DiffUtil {
  ...
  private static Snake diffPartial(Callback paramCallback, int paramInt1, int paramInt2, int paramInt3, int paramInt4, int[] paramArrayOfint1, int[] paramArrayOfint2, int paramInt5) {
    ...
    if (paramInt2 < 1 || m < 1)
      return null; 
    int j = paramInt2 - m;
    int k = (paramInt2 + m + 1) / 2;
    int i = paramInt5 - k - 1;
    paramInt4 = paramInt5 + k + 1;
    Arrays.fill(paramArrayOfint1, i, paramInt4, 0);
    Arrays.fill(paramArrayOfint2, i + j, paramInt4 + j, paramInt2);
    if (j % 2 != 0) {
      i = 1;
    } else {
      i = 0;
  ...
  public static class DiffResult {
    ...
    private void dispatchRemovals(List<DiffUtil.PostponedUpdate> param1List, ListUpdateCallback param1ListUpdateCallback, int param1Int1, int param1Int2, int param1Int3) {
      ...
      while (--param1Int2 >= 0) {
        ...
        int j = param1Int3 + param1Int2;
        int i = arrayOfInt[j] & 0x1F;
        if (i != 0) {
          if (i != 4 && i != 8) {
            if (i == 16) {
              param1List.add(new DiffUtil.PostponedUpdate(j, param1Int1 + param1Int2, true));
            } else {
              stringBuilder = new StringBuilder();
              stringBuilder.append("unknown flag for pos ");
            ...
            int k = this.mOldItemStatuses[j] >> 5;
            ...
            if (i == 4)
              param1ListUpdateCallback.onChanged(postponedUpdate.currentPos - 1, 1, this.mCallback.getChangePayload(j, k)); 
          } 
        } else {
...
/* Location:              C:\Users\admin\Desktop\Test\dex-tools-v2.4\classes-dex2jar.jar!\androidx\recyclerview\widget\DiffUtil.class
