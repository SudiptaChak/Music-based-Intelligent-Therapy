...
public class ConstraintTableLayout extends ConstraintWidgetContainer {
  ...
  private void setChildrenConnections() {
    ...
    int i = 0;
    while (b < j) {
      ConstraintWidget constraintWidget3 = this.mChildren.get(b);
      i += constraintWidget3.getContainerItemSkip();
      int m = this.mNumCols;
      int k = i / m;
      HorizontalSlice horizontalSlice = this.mHorizontalSlices.get(k);
      VerticalSlice verticalSlice = this.mVerticalSlices.get(i % m);
      ConstraintWidget constraintWidget4 = verticalSlice.left;
      ConstraintWidget constraintWidget1 = verticalSlice.right;
      ConstraintWidget constraintWidget2 = horizontalSlice.top;
      ConstraintWidget constraintWidget5 = horizontalSlice.bottom;
      constraintWidget3.getAnchor(ConstraintAnchor.Type.LEFT).connect(constraintWidget4.getAnchor(ConstraintAnchor.Type.LEFT), this.mPadding);
      if (constraintWidget1 instanceof Guideline) {
        constraintWidget3.getAnchor(ConstraintAnchor.Type.RIGHT).connect(constraintWidget1.getAnchor(ConstraintAnchor.Type.LEFT), this.mPadding);
      } else {
        constraintWidget3.getAnchor(ConstraintAnchor.Type.RIGHT).connect(constraintWidget1.getAnchor(ConstraintAnchor.Type.RIGHT), this.mPadding);
      } 
      k = verticalSlice.alignment;
      if (k != 1) {
        if (k != 2) {
          if (k == 3)
            constraintWidget3.setHorizontalDimensionBehaviour(ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT); 
        } else {
          constraintWidget3.getAnchor(ConstraintAnchor.Type.LEFT).setStrength(ConstraintAnchor.Strength.WEAK);
          constraintWidget3.getAnchor(ConstraintAnchor.Type.RIGHT).setStrength(ConstraintAnchor.Strength.STRONG);
        } 
      } else {
        constraintWidget3.getAnchor(ConstraintAnchor.Type.LEFT).setStrength(ConstraintAnchor.Strength.STRONG);
        constraintWidget3.getAnchor(ConstraintAnchor.Type.RIGHT).setStrength(ConstraintAnchor.Strength.WEAK);
      } 
      constraintWidget3.getAnchor(ConstraintAnchor.Type.TOP).connect(constraintWidget2.getAnchor(ConstraintAnchor.Type.TOP), this.mPadding);
      if (constraintWidget5 instanceof Guideline) {
        constraintWidget3.getAnchor(ConstraintAnchor.Type.BOTTOM).connect(constraintWidget5.getAnchor(ConstraintAnchor.Type.TOP), this.mPadding);
      } else {
        constraintWidget3.getAnchor(ConstraintAnchor.Type.BOTTOM).connect(constraintWidget5.getAnchor(ConstraintAnchor.Type.BOTTOM), this.mPadding);
      } 
      i++;
      b++;
    } 
  ...
  public void setTableDimensions() {
    ...
    int i = 0;
    while (j < k) {
      i += ((ConstraintWidget)this.mChildren.get(j)).getContainerItemSkip();
      j++;
    } 
    k += i;
...
/* Location:              C:\Users\admin\Desktop\Test\dex-tools-v2.4\classes-dex2jar.jar!\androidx\constraintlayout\solver\widgets\ConstraintTableLayout.class
