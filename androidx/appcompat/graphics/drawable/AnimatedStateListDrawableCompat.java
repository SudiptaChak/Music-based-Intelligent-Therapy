...
public class AnimatedStateListDrawableCompat extends StateListDrawable implements TintAwareDrawable {
  ...
  private boolean selectTransition(int paramInt) {
    ...
    int m = animatedStateListState.getKeyframeIdAt(paramInt);
    if (m == 0 || k == 0)
      return false; 
    int j = animatedStateListState.indexOfTransition(k, m);
    if (j < 0)
      return false; 
    boolean bool = animatedStateListState.transitionHasReversibleFlag(k, m);
    selectDrawable(j);
    Drawable drawable = getCurrent();
    if (drawable instanceof AnimationDrawable) {
      boolean bool1 = animatedStateListState.isTransitionReversed(k, m);
...
/* Location:              C:\Users\admin\Desktop\Test\dex-tools-v2.4\classes-dex2jar.jar!\androidx\appcompat\graphics\drawable\AnimatedStateListDrawableCompat.class
